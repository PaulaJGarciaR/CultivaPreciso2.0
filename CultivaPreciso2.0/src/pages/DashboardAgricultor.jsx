...
export default function DashboardAgricultor({ user }) {
  ...
  const getInitials = () => {
    ...
    return user.displayName.split(" ").slice(0, 2).map((n) => n[0]).join("").toUpperCase();
  };
  ...
  const renderView = () => {
    switch (activeNav) {
      ...
      case "cultivo":      return <ViewCultivo cultivo={cultivo} setCultivo={setCultivo} user={user} />;
      case "weather":      return <ViewWeather cultivo={cultivo} />;
      case "monitoring":   return <ViewMonitoring />;
      case "sensores":     return <ViewCalculadoraSensores />;
      case "reports":      return <ViewReports cultivo={cultivo} />;
      case "calendar":     return <ViewCalendar cultivo={cultivo} user={user} />;
      case "enfermedades": return <ViewEnfermedades />;
      default:             return null;
    }
  };
    ...
    return (
      <button
        ...
        className={`nav-item w-full flex items-center gap-3 px-4 py-2.5 text-left
          ${active
            ? fito ? "nav-item-fito active" : "active"
