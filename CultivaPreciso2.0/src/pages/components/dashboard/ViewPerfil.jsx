...
export default function ViewPerfil({ user, userData, setUserData }) {
  ...
  useEffect(() => {
    if (userData) {
      ...
      setSecondSurname(userData.secondSurname || "");
    }
  }, [userData]);
  ...
  const handleSaveInfo = async () => {
    ...
    try {
      ...
      await updateProfile(auth.currentUser, {
        displayName: `${firstName} ${middleName ? middleName + " " : ""}${lastName}${secondSurname ? " " + secondSurname : ""}`,
      });
      setUserData((p) => ({
        ...p,
        firstName,
  ...
  const displayName =
    `${firstName} ${middleName ? middleName + " " : ""}${lastName}${secondSurname ? " " + secondSurname : ""}`.trim() ||
    user?.email?.split("@")[0];
  const initials =
    firstName && lastName
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-6 pb-10">
      ...
      <div className="flex items-center gap-5">
        <div className="w-20 h-20 rounded-full bg-[#CC9633]/20 flex items-center justify-center shrink-0">
          {user?.photoURL ? (
            ...
            <span className="text-[#CC9633] text-2xl font-bold">
              {initials}
            </span>
          )}
        </div>
          ...
          {isGoogle && (
            ...
            >
              <svg width="12" height="12" viewBox="0 0 48 48">
                <path
                  fill="#FFC107"
                  d="M43.6 20H24v8h11.3C33.7 32.7 29.2 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.8 1.2 8 3l5.7-5.7C34 6 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.7-.4-4z"
                />
                <path
                  fill="#FF3D00"
                  d="M6.3 14.7l6.6 4.8C14.7 15.1 19 12 24 12c3 0 5.8 1.2 8 3l5.7-5.7C34 6 29.3 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"
                />
                <path
                  fill="#4CAF50"
                ...
                <path
                  fill="#1976D2"
                  d="M43.6 20H24v8h11.3c-.8 2.2-2.2 4.2-4.1 5.6l6.2 5.2C41 35.2 44 30 44 24c0-1.3-.1-2.7-.4-4z"
                />
              </svg>
              Cuenta de Google
            </span>
      ...
      <SectionCard
        title="Mi cultivo guardado"
        icon={<span className="text-base">🌱</span>}
      >
        {loadingCultivo ? (
          <p className="text-[#2F281F]/50 text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              {
                label: "Nombre",
                value: cultivoPerfil.nombre || "Sin registrar",
              },
              {
                label: "Área",
                value: cultivoPerfil.hectareas
                  ? `${cultivoPerfil.hectareas} ha`
                  : "Sin registrar",
              },
              {
                label: "Variedad",
                value: cultivoPerfil.variedad || "Sin registrar",
              },
              {
                label: "Siembra",
                value: cultivoPerfil.fechaSiembra || "Sin registrar",
              },
              {
                label: "Región",
                value: cultivoPerfil.region || "Sin registrar",
              },
              {
                label: "Última actualización",
                value: cultivoPerfil.actualizadoEn
                  ? new Date(cultivoPerfil.actualizadoEn).toLocaleString(
                      "es-CO",
                    )
                  : "Sin registrar",
              },
              ...
              >
                <p className="text-[#2F281F]/45 text-[10px] uppercase tracking-widest">
                  {item.label}
                </p>
                <p className="text-[#2F281F] text-sm font-semibold mt-1">
                  {item.value}
                </p>
              </div>
            ))}
            {cultivoPerfil.notas && (
              ...
              >
                ...
                <p className="text-[#2F281F]/75 text-sm mt-1">
                  {cultivoPerfil.notas}
                </p>
              </div>
            )}
          ...
          <p className="text-[#2F281F]/50 text-sm">
            Aún no hay datos de cultivo guardados. Completa la pestaña Mi
            Cultivo y se guardará automáticamente.
          </p>
        )}
      </SectionCard>
      ...
      >
        <p className="text-white/40 text-sm">
          Eliminar tu cuenta es una acción permanente. Se borrarán todos tus
